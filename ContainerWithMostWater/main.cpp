...
public:
    int maxArea(std::vector<int>& height) {
        ...
        for(int i = 1, z = height.size()-2; i < height.size(); i++, z--)
        {
            int limit1 = height[j] < height[w] ? height[j] : height[w];
            if(limit1*abs(w-j) >= max)
            {
                max = limit1*abs(w-j);
            }
            int limit2 = height[z] < height[i] ? height[z] : height[i];
            if(limit2*abs(z-i) >= max)
            {
                max = limit2*abs(z-i);
            }

            int limit3 = height[j] < height[i] ? height[j] : height[i];
            if(limit3*abs(i-j) >= max)
            {
                max = limit3*abs(i-j);
            }

            int limit4 = height[w] < height[z] ? height[w] : height[z];
            if(limit4*abs(w-z) >= max)
            {
                max = limit4*abs(w-z);
            }
            ...
            //limit 5 i-w
            //limit 6 j-z
            int limit5 = height[i] < height[w] ? height[i] : height[w];
            if(limit5*abs(w-i) >= max)
            {
                max = limit5*abs(w-i);
            }

            int limit6 = height[j] < height[z] ? height[j] : height[z];
            if(limit6*abs(z-j) >= max)
            {
                max = limit6*abs(z-j);
            }

            if(height[i] - height[j] > abs(i-j) )
            {
                j = i;
            }

            if(height[z] - height[w] > abs(z-w))
            {
                w = z;
            }
        }
